...
import { DeviceListInfoItem, DeviceTourvouchGetById, deleteDeviceVouchFile } from '@/api/PDA.js'
...
const ToGtagrt = (i) => {
  ...
  uni.navigateTo({
    url: `/pages/ProEquipmentCheck/ProPointTarget/ProPointTarget?uid=${i.UID}`
  })
}
// 文档
const goFile = (i) => {
  if (i.cProgramName) {
    uni.navigateTo({
      url: `/pages/ProEquipmentCheck/ProEquipFile/ProEquipFile?code=${i.cProgramCode}`
    })
  } else {
    uni.navigateTo({
      url: `/pages/ProEquipmentCheck/ProEquipFile/ProEquipFile?Devcode=${code.value}`
    })
  }
}
// 报修
const ToRepairt = (i) => {
  console.log(i, '项目----------')
  uni.navigateTo({
    url: `/pages/ProEquipmentCheck/ProEquipmentRepair/ProEquipmentRepair?code=${code.value}&name=${name.value}&addres=${address.value}&program=${i.cProgramName}&progeamCode=${i.cProgramCode}&cPar01=${Parm.value}&cParm03=${cParmCode.value}`
  })
}
//单据增加图片
const ToPic = (i) => {
