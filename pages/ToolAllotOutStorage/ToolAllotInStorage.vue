            ...
            >
              <view class="wLIst">
                ...
                <uni-row class="demo-uni-row">
                  ...
                  <uni-col :span="16">
                    <view class="demo-uni-col dark">数量：{{ item.Rest_InQuantity }}</view>
                  </uni-col>
                </uni-row>
...
import { getAllotInList } from '@/api/PDA_3.js'
...
const getWaresList = async () => {
  ...
  const res = await getAllotInList({
    PageIndex: currentPage.value,
    pageSize: pageSize.value,
    OrderByFileds: '',
    Conditions: InfosearchValue.value
      ? `Rest_InQuantity>0 && Out_Quantity>0 & cCode like ${InfosearchValue.value}`
      : `Rest_InQuantity>0 && Out_Quantity>0`
  })
  if (res.status == 200) {
    uni.hideLoading()
...
const goInWare = (i) => {
  uni.navigateTo({
    url: '/pages/ToolAllotOutStorage/OpearAllotIn/OpearAllotIn?itemInfo=' + JSON.stringify(i)
  })
}
//下拉
onPullDownRefresh(() => {
