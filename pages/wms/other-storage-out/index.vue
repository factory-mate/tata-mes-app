...
import { OtherStorageOutAPI, WareHouseAPI } from '@/api'
...
async function getList() {
  ...
  try {
    ...
    } = await OtherStorageOutAPI.GetForPage_OtherOut_PDA({
      PageIndex: pageParams.value.pageIndex,
      PageSize: pageParams.value.pageSize,
      OrderByFileds: '',
      Conditions: queryBuilder([
        { type: 'like', key: 'cCode', val: keyword.value },
        { type: 'eq', key: 'cOutWareHouseCode', val: storageData.value?.cWareHouseCode }
      ])
    })
...
const navToMaterial = (item) => {
  ...
  uni.navigateTo({
    url: `/pages/wms/other-storage-out/material?id=${item.UID}&code=${storageData.value.cWareHouseCode}`
  })
}
      ...
      >
        ...
        <view style="padding-top: 120px">
          ...
          >
            ...
            <up-gap height="8" />
            <up-row justify="space-between">
              <up-col span="6"> 出库仓库：{{ item.cOutWareHouseCode }} </up-col>
