        ...
        >
          ...
          <view style="font-size: 14px; margin: 10px">
            <view
              ...
              >{{ item.FileName1 ? item.FileName1?.split('&')[0] : '' }}</view
            >
            <view
              style="font-size: 14px; margin: 10px"
              @click="preview(2, item.FileName2)"
              >{{ item.FileName1 ? item.FileName2?.split('&')[0] : '' }}</view
            >
            <view
              style="font-size: 14px; margin: 10px"
              @click="preview(3, item.FileName3)"
              >{{ item.FileName1 ? item.FileName3?.split('&')[0] : '' }}</view
            >
          </view>
...
const ToGtagrt = (i) => {
  console.log(i, 'i-----------')
  uni.navigateTo({
    url: `/pages/DeviceMantin/ManTainTarget/ManTainTarget?uid=${i.UID}&code=${code.value}&partcode=${i.cProgramCode}`
  })
}
// 文档
const goFile = (i) => {
...
const ToRepairt = (i) => {
  uni.navigateTo({
    url: `/pages/DeviceMantin/DevRepair/DevRepair?code=${code.value}&name=${name.value}&addres=${address.value}&program=${i.cProgramName}&progeamCode=${i.cProgramCode}&cPar01=${Parm.value}&cParm03=${cParmCode.value}`
  })
}
//单据增加图片
const ToPic = (i) => {
  uni.navigateTo({
    url: `/pages/DeviceMantin/AddPicture/AddPicture?uid=${i.UID}`
  })
}
//设备添加图片
const DEVpic = () => {
  uni.navigateTo({
    url: `/pages/DeviceMantin/AddPicture/AddPicture?Devuid=${uid.value}`
  })
}
//备件
const ToPart = (i) => {
  uni.navigateTo({
    url: `/pages/DeviceMantin/ReadyPart/ReadyPart?id=${i.UID}`
  })
}
//获取数据
const getList = () => {
