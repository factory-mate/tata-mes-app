        ...
        >
          ...
          >
            <view class="wLIst">
              ...
              <uni-row class="demo-uni-row">
                <uni-col :span="10">
                  <view class="demo-uni-col dark">设备类型：{{ item.cSourceAppType }}</view>
                </uni-col>
                <uni-col :span="16">
...
const GetList = () => {
  ...
  STOCKTAKEPage({
    ...
    OrderByFileds: '',
    Conditions: searchValue.value
      ? `cvouchTypeCode=1 & cCode=${searchValue.value}`
      : 'cvouchTypeCode=1'
  }).then((res) => {
    if (res.status == 200) {
...
const goView = (i) => {
  console.log(i, 'i--------------')
  uni.navigateTo({
    url: `/pages/checkBills/checkView/checkView?id=${i.UID}`
  })
}
// 删除
