...
<script setup>
import { ref } from 'vue'
import { onLoad, onShow, onUnload, onHide, onBackPress, onPullDownRefresh } from '@dcloudio/uni-app'
...
const getCangku = () => {
  if (xiangMList.value.length) {
    if (CKval.value != xiangMList.value[0].cWareHouseCode) {
      uni.showToast({
        icon: 'none',
        title: '仓库不对'
      })
...
const getDanList = () => {
  let obj = {
    ...
    Conditions: dhVal.value
      ? 'cVouchSourceTypeCode=002 && iStatus=0 && cCode like ' + dhVal.value
      : 'cVouchSourceTypeCode=002 && iStatus=0'
  }
  uni.showLoading({
    title: '加载中'
  })
...
const clickShaom = (v) => {
  ...
  cCode.value = v.cCode
  UID.value = v.UID
  CKval.value = v.cDefindParm02
  changkuData.value = {
    cWareHouseCode: v.cDefindParm02,
    cWareHouseName: v.cDefindParm03
  }
}
const clickAdd = () => {
  uni.showLoading({
