...
onLoad((option) => {
  ...
  partcode.value = option.partcode
  GetDataTarget()
})
const keypress = (e) => {
  // codeType.value = ''
...
const GetDataTarget = () => {
  ...
  GetMaintainTarget({
    OrderByFileds: '',
    Conditions: ` cDeviceCode=${code.value} && cProgramCode=${partcode.value}`
  }).then((res) => {
    if (res.status == 200) {
...
const checkTarget = (i) => {
  uni.navigateTo({
    url:
      `/pages/ProDeviceMantin/ProTargetList/ProTargetList` +
      `?cDeviceStandardCode=${i.cDeviceStandardCode}`
  })
}
//下拉
onPullDownRefresh(() => {
