...
import { GetMaintainTarget } from '@/api/PDA.js'
...
const uid = ref()
const cDeviceCode = ref()
const cProgramCode = ref()
onShow(() => {
...
onLoad((option) => {
  ...
  cDeviceCode.value = option.cDeviceCode
  cProgramCode.value = option.cProgramCode
  GetDataTarget()
})
const keypress = (e) => {
  // codeType.value = ''
...
const GetDataTarget = () => {
  ...
  let Conditions = ''
  const conditionList = []
  if (cDeviceCode.value) {
    conditionList.push(`cDeviceCode = ${cDeviceCode.value}`)
  }
  if (cProgramCode.value) {
    conditionList.push(`cProgramCode = ${cProgramCode.value}`)
  }
...
const checkTarget = (i) => {
  uni.navigateTo({
    url: `/pages/ProDeviceMantin/ProTargetList/ProTargetList`
  })
}
//下拉
onPullDownRefresh(() => {
