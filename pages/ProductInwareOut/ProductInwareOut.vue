    ...
    <view class="content">
      <view class="listMain">
        ...
        >
          <view
            class="project"
            v-for="(item, index) in OutListData"
            :key="index"
          >
            ...
            <uni-row class="demo-uni-row">
              <uni-col :span="10">
                <view class="demo-uni-col dark">客户：{{ item.cDefindParm02 }}</view>
              </uni-col>
              <uni-col :span="16">
            ...
            <uni-row class="demo-uni-row">
              <uni-col :span="16">
                <view class="demo-uni-col dark">销售订单：{{ item.cDefindParm01 }}</view>
              </uni-col>
              <uni-col :span="8">
...
import { GetWare, ProductOutList } from '@/api/PDA.js'
...
const GetList = async () => {
  ...
  OutListData.value = []
  const res = await ProductOutList({
    PageIndex: currentPage.value,
    PageSize: pageSize.value,
    OrderByFileds: '',
    Conditions: InfosearchValue.value
      ? `cOutWareHouseCode=${WsearchValue.value} && cCode=${InfosearchValue.value}`
      : `cOutWareHouseCode=${WsearchValue.value}`
  })
  if (res.status == 200) {
    InfosearchValue.value = ''
    uni.hideLoading()
    uni.stopPullDownRefresh()
    OutListData.value = [...OutListData.value, ...res.data.data]
...
const GetDetail = (i, index) => {
  ...
  uni.navigateTo({
    url: '/pages/ProductInwareOut/ProductOutDetail/ProductOutDetail?detail=' + detail
  })
}
//下拉
onPullDownRefresh(() => {
