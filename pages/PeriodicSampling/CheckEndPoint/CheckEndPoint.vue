        ...
        >
          ...
          >
            <view>
              ...
              <uni-row class="demo-uni-row">
                <uni-col :span="12">
                  <view class="demo-uni-col dark">扣分：{{ item.nSore }}</view>
                </uni-col>
                <uni-col :span="12">
...
import { GetListBYMID } from '@/api/Quanilty.js'
...
onLoad((option) => {
  ...
  InfoItem.value = JSON.parse(option.Info)
  GetList()
})
const setfocus = () => {
...
const GoDection = (i) => {
  if (i.cAnalysisTypeName == '定性') {
    //定性
    uni.navigateTo({
      url: '/pages/PeriodicSampling/EndQuaniltyPoint/EndQuaniltyPoint?ItemInfo=' + JSON.stringify(i)
    })
  } else {
    //定量
    uni.navigateTo({
      url:
        '/pages/PeriodicSampling/EndQuanilyNumPoint/EndQuanilyNumPoint?ItemInfo=' +
        JSON.stringify(i)
    })
  }
}
