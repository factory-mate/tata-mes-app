...
const clckType = (v) => {
  if (v.cAnalysisTypeName == '定量') {
    uni.navigateTo({
      url: '/pages/kuIncomingInspection/InspectingPackingViewDL?obj=' + JSON.stringify(v)
    })
  } else {
    uni.navigateTo({
      url: '/pages/kuIncomingInspection/InspectingPackingViewDX?obj=' + JSON.stringify(v)
    })
  }
}
const getList = () => {
