    ...
    <view style="box-sizing: border-box; padding: 0 10rpx; border: 1px solid #eee">
      ...
      <uni-row class="demo-uni-row formSty">
        <uni-col :span="24">
          <view
            class="demo-uni-col"
            @click="preview(1, getByVal.FileList)"
            >附件：{{
      ...
      <view style="display: flex; align-items: center; font-size: 12px; margin-bottom: 20rpx">
        <view style="width: 90px">检测值：</view>
        <view
          class="search-inpt"
          style="width: 60%"
        >
...
const getList = () => {
  ...
  GetForList_S_S(obj).then((res) => {
    ...
    if (res.data[0]) {
      jcVal.value = res.data[0].nCheckValue
      memo.value = res.data[0].cMemo
    }
    // pageTotal.value=res.data.pageCount
  })
}
