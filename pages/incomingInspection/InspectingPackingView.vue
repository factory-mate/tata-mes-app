...
const codeVal = ref('')
const xmVal = ref('')
const nDestoryQuantity = ref(null)
...
const getId = () => {
  GetById({ val: routeVal.value.UID }).then((res) => {
    ...
    nDestoryQuantity.value = res.data.nDestoryQuantity
  })
}
//货位输入框聚焦
const setfocus = () => {
...
const zhibiao = (obj) => {
  uni.navigateTo({
    url: '/pages/incomingInspection/InspectingPackingViewZB?obj=' + JSON.stringify(obj)
  })
}
const handleScan = () => {
  toScanCode({
