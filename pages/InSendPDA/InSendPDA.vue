<template>
	<view class="changeList">
		<uni-nav-bar dark shadow background-color="red" status-bar left-icon="left" :title="title"
			@clickLeft="clickLeft" />
			<view>
				<view class="listMain"  :style="'height:'+ (h-140) + 'px'">
					<view class="project"  v-for="(item,index) in DataList" :key="index">
						<uni-row class="demo-uni-row">
							<uni-col :span="10">
								<view class="demo-uni-col dark" style="font-size: 16px;color: blue;">申报人：{{item.max_Error_dReportPerson}}</view>
							</uni-col>
							<uni-col :span="14">
								<view class="demo-uni-col dark" style="font-size: 16px;color: blue;">申报日期：{{item.max_Error_dReportDate}}</view>
							</uni-col>
						</uni-row>
						<uni-row class="demo-uni-row">
							<uni-col :span="16">
						...
						<view class="Two-btn">
							<button class="mini-btn" style="background-color: blue;" type="warn" size="mini" @click="goReapirReturn(item)">返修</button>
							<button class="mini-btn" style="background-color: green;" type="warn" size="mini" @click="Close(item)">关闭</button>
						</view>
					</view>
				</view>
			</view>
