            ...
            >
              <view class="wLIst">
                <uni-row class="demo-uni-row">
                  ...
                  <uni-col :span="16">
                    <view class="demo-uni-col dark">报废单号：{{ item.cCode }}</view>
                  </uni-col>
                </uni-row>
                <uni-row class="demo-uni-row">
                  <uni-col :span="14">
...
import { GetForPage_PDA_Out } from '@/api/baofei.js'
...
const getWaresList = async () => {
  ...
  await GetForPage_PDA_Out({
    PageIndex: currentPage.value,
    pageSize: pageSize.value,
    OrderByFileds: '',
    Conditions: InfosearchValue.value
      ? 'Rest_OutQuantity>0 && iStatus=1 && cCode=' + InfosearchValue.value
      : 'Rest_OutQuantity>0 && iStatus=1'
  }).then((res) => {
    if (res.status == 200) {
      uni.hideLoading()
...
const goInWare = (obj) => {
  uni.navigateTo({
    url: '/pages/ToolScarp/ScarpOpear/ScarpOpear?obj=' + JSON.stringify(obj)
  })
}
//下拉
onPullDownRefresh(() => {
