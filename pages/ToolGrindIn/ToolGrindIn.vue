...
const getWaresList = async () => {
  ...
  await GetPDAInOrOutListData({
    PageIndex: currentPage.value,
    pageSize: pageSize.value,
    OrderByFileds: '',
    Conditions: InfosearchValue.value
      ? `Out_Quantity>0 && Rest_InQuantity>0 && cCode=${InfosearchValue.value}`
      : 'Out_Quantity>0 && Rest_InQuantity>0'
  }).then((res) => {
    if (res.status == 200) {
      uni.hideLoading()
...
const goInWare = (obj) => {
  uni.navigateTo({
    url: '/pages/ToolGrindIn/ToolGrindOpear/ToolGrindOpear?obj=' + JSON.stringify(obj)
  })
}
//下拉
onPullDownRefresh(() => {
