<template>
	<view class="da-tree" :style="{'--theme-color': themeColor}">
		...
		<view class="da-tree-item" :class="{'is-show': item.show}" :style="{paddingLeft: item.level * indent + 'rpx'}"
			...
			<view class="da-tree-item__label"  @click="handleLabelClick(item)">
				<view :class="'da-tree-item__label--'+item.checkedStatus">
					{{ item.label }} <text class="da-tree-item__label--append"
						v-if="item.append">{{ item.append }}</text>
				</view>
				<!-- 右侧下拉选择 -->
				<!--  -->
				<view style="margin-left:40% ;" v-if="item.originItem.cLineName">
					<uni-data-select style="width: 90px;" v-model="item.originItem.cLineCode" :placeholder="item.originItem.cLineName"
						:localdata="range" @click="changess(item)" @change="change" :clear="true"></uni-data-select>
				</view>
			</view>
...
<script>
	...
	export default defineComponent({
		...
		setup(props, {
			...
			// 下拉选项
			const changess = async (i) => {
				const res = await GetProductLine({
					"OrderByFileds": "cCraftRouteCode",
					"Conditions": `cCraftRouteCode=${i.parentKey}`
				})
				if (res.status == 200) {
					range.value = res.data[0].resources.map(item => {
						return {
							value: item.cResourceCode,
							text: item.cResourceName
						}
					})
				}
			}
			let LineNameArr = []
			const change = (i) => {
				let LineName = range.value.filter(item => item.value == i)
				LineNameArr=[...LineNameArr,...LineName]
				uni.setStorageSync('LineName', LineNameArr)
			}
			/**
			 * 点击展开收起
			 * @param item
